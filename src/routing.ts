...
export interface ScheduleResult {
  readonly immediate: readonly RoutedEvent[];
...
export class Scheduler {
  ...
  schedule(event: RoutedEvent): ScheduleResult {
    const destination = event.decision.destination;
    try {
      if (destination === "pending") {
        this.pending.put(event);
        return { immediate: [], queued: true, rejected: false, reason: "" };
      }
      if (destination === "held") {
        this.held.put(event);
        return { immediate: [], queued: true, rejected: false, reason: "" };
      ...
      this.deadLetters.put(event.event, reason);
      return { immediate: [], queued: false, rejected: true, reason };
    }
    if (destination === "drop") {
      this.deadLetters.put(event.event, event.decision.reason);
      return {
        immediate: [],
        queued: false,
        rejected: true,
    ...
    return { immediate: [event], queued: false, rejected: false, reason: "" };
  }

  safePoint(taskId: string): RoutedEvent[] {
    return this.pending.drainCompatible({ taskId });
  }
}
