...
`laohuang` 是纯 npm 制品：`tsc` 把 `src/` 编译到 `dist/`，`npm publish` 发布包含
`dist/`、`README.md` 和 `LICENSE` 的 tarball。没有 wheel、venv 或 PyPI 环节。
...
1. 在功能 pull request 中同时修改 `package.json` 和 `package-lock.json` 中的
   版本。当前发布只接受稳定的 `X.Y.Z` SemVer。
2. 本地执行完整检查：
