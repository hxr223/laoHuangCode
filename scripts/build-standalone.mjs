...
try {
  ...
  if (platform === 'win32') {
    const zipScript = join(stage, 'create-zip.ps1');
...
`);
    // The output is generated by this build; allow an explicit local rebuild.
    rmSync(archive, { force: true });
    run('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', zipScript, packageDir, archive]);
    if (readFileSync(archive).readUInt32LE(0) !== 0x04034b50) throw new Error('Standalone archive is not ZIP');
