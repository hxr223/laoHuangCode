...
原生 macOS 使用 `Ctrl+V`，原生 Windows 使用 `Alt+V`：优先读取系统剪贴板图片，
保存为临时 PNG，并在输入框光标处插入带引号的路径；没有图片时粘贴文本。
可在路径旁写“读取这张图片”，提交后由 Agent 调用 `read_image`，不是粘贴时直接发送图片。
macOS 的 `Cmd+V`、Windows 终端自己的文本粘贴行为不变。

图片遵循附件源文件限制（20 MiB、6400 万像素、最长边 8192），文本限 1 MiB。
读取失败会提示，不会自动提交；读取期间修改输入、打开弹窗或切换会话会取消本次粘贴。
临时图片在当前 CLI 退出时清理，`read_image` 已导入的附件不受影响；未被读取的临时路径
不保证退出后可用。异常退出的临时目录由系统临时目录策略处理。
该系统剪贴板入口仅适配原生 macOS/Windows，Linux、WSL、SSH 请使用运行主机上的图片路径。
