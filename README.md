...
`read` 每次最多返回 50 KiB（51,200 字节）的 UTF-8 正文、2,000 行，每行最多
2,000 个 Unicode 字符（不含换行符）。`limit` 默认 2,000，不能超过该上限。
结果中的 `next_offset` 指向下一条未返回的行，到文件末尾时为 `null`；
`truncated_line_numbers` 标出被裁短的行，可通过 Bash 读取这些行的剩余内容。
只要存在未返回的后续行或被裁短的行，`has_more` 就为 `true`。
